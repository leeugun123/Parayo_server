...
class InquirySearchService  @Autowired constructor(
    ...
    fun getProductInquiries(
        ...
        return when(condition){
            ...
            NEXT_FOR_PRODUCT -> inquiryRepository.findByProductIdAndIdLessThanOrderByIdDesc(
                productId, inquiryId, PageRequest.of(0, 10)
            )
            PREV_FOR_PRODUCT -> inquiryRepository.findByProductIdAndIdGreaterThanOrderByIdDesc(
                productId, inquiryId, PageRequest.of(0, 10)
            )
            NEXT_FOR_USER -> inquiryRepository.findByRequestUserIdAndIdLessThanOrderByIdDesc(
                requestUserId, inquiryId, PageRequest.of(0, 10)
            )
            PREV_FOR_USER -> inquiryRepository.findByRequestUserIdAndIdGreaterThanOrderByIdDesc(
                requestUserId, inquiryId, PageRequest.of(0, 10)
            )
            NEXT_FOR_USER_PRODUCT -> inquiryRepository.findByProductOwnerIdAndIdLessThanOrderByIdDesc(
                productOwnerId, inquiryId, PageRequest.of(0, 10)
            )
            PREV_FOR_USER_PRODUCT -> inquiryRepository.findByProductOwnerIdAndIdGreaterThanOrderByIdDesc(
                productOwnerId, inquiryId, PageRequest.of(0, 10)
            )
